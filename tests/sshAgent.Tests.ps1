...
Describe "[$global:IMAGE_NAME] image has correct version of tools installed and in the PATH" {
    ...
    It 'has sshd installed and in the path' {
        $exitCode, $stdout, $stderr = Run-Program 'docker' "exec $global:CONTAINERNAME $global:CONTAINERSHELL -C `"if(`$null -eq (Get-Command sshd.exe -ErrorAction SilentlyContinue)) { exit -1 } else { exit 0 }`""
        $exitCode | Should -Be 0

        $exitCode, $stdout, $stderr = Run-Program 'docker' "exec $global:CONTAINERNAME $global:CONTAINERSHELL -C `"`$version = sshd -V 2>&1 ; Write-Host `$version`""
        $exitCode | Should -Be 0
...
Describe "[$global:IMAGE_NAME] create agent container with pubkey as envvar" {
    BeforeAll {
        $exitCode, $stdout, $stderr = Run-Program 'docker' "run --detach --name=`"$global:CONTAINERNAME`" --publish-all --env=`"JENKINS_AGENT_SSH_PUBKEY=$global:PUBLIC_SSH_KEY`" `"$global:IMAGE_NAME`""
        $exitCode | Should -Be 0
        Is-ContainerRunning $global:CONTAINERNAME | Should -BeTrue
        Start-Sleep -Seconds 10
    }
...
Describe "[$global:IMAGE_NAME] build args" {
    ...
    It 'uses build args correctly' {
        ...
        $exitCode, $stdout, $stderr = Run-Program 'docker' "run --detach --name=`"$global:CONTAINERNAME`" --publish-all `"$CUSTOM_IMAGE_NAME`""
        $exitCode | Should -Be 0
        Is-ContainerRunning "$global:CONTAINERNAME" | Should -BeTrue
