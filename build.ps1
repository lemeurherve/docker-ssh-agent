...
$generateDockerCompose = $OverwriteDockerComposeFile
...
if ($generateDockerCompose) {
    Write-Host '= PREPARE: generating the docker compose file "{0}" containing the image definitions from "{1}":' -f $dockerComposeFile, $dockerBakeFile
    ...
    # - Use docker buildx bake to output image definitions from the "<windowsFlavor>" docker bake target
    # - Convert with yq to the format expected by docker compose
    # - Store the result in the docker compose file
    $generateDockerComposeFileCmd = ' {0} {1} --print' + `
        ' | yq --pretty {3} | yq {4}' + `
        ' | Out-File -FilePath {5}' -f $baseDockerBakeCmd, $windowsFlavor, $yqMainQuery, $yqServicesQuery, $dockerComposeFile
